		...
		this.control({
			...
			"container[xtype=menu.menutree] button[ref=treeIns]":{
				click:function(btn){
					...
					var resObj=self.ajax({url:"/pcMenu/doSave.action",params:params});
					if(resObj.success){
			...
			"container[xtype=menu.menutree] button[ref=treeDel]":{
				click:function(btn){
					...
					var node=records[0];
					var resObj=self.ajax({url:"/pcMenu/doRemove.action",params:{ids:node.get("id")}});
					if(resObj.success){
						tree.getStore().load();
